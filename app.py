...
@login_required
def dashboard():
    # ✅ Show all appointments of current user (static user_id=1 here)
    with SessionLocal() as db:
        appointments = db.query(Appointment).filter_by(user_id=1).all()
    return render_template("dashboard.html", username=session.get("username", "User"), appointments=appointments)
...
def predict():
    ...
    # Store result data in session for the template
    session["last_result"] = {
...
@app.route("/api/result-data")
@login_required
def api_result_data():
    # Get the latest result data from session
...
def cancel_appointment(appt_id):
    with SessionLocal() as dbs:
        appt = dbs.query(Appointment).filter_by(id=appt_id, user_id=1).first()
        if appt:
            dbs.delete(appt)
            dbs.commit()
            flash(f"Appointment {appt_id} has been cancelled.", "warning")
        else:
            flash("Appointment not found or already cancelled.", "danger")
    return redirect(url_for("dashboard"))
